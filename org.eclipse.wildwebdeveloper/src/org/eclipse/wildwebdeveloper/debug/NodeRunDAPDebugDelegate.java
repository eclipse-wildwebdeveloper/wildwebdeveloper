...
public class NodeRunDAPDebugDelegate extends DSPLaunchDelegate {
	...
	public void launch(ILaunchConfiguration configuration, String mode, ILaunch launch, IProgressMonitor monitor) throws CoreException {
		...
		String argsString = wc.getAttribute(ARGUMENTS, "").trim();
		if (!argsString.isEmpty()) {
			JsonArray args = new JsonArray();
			Arrays.asList(argsString.split(" ")).stream()
		...
		String cwd = wc.getAttribute(DebugPlugin.ATTR_WORKING_DIRECTORY, "").trim();
		if (!cwd.isEmpty()) {
			param.addProperty(CWD, cwd);
		}
		// internal settings
		if (!ILaunchManager.DEBUG_MODE.equals(mode)) {
			param.addProperty("noDebug", true);
		}
		...
		try {
			...
			ErrorDialog.openError(Display.getDefault().getActiveShell(), "Debug error", e.getMessage(), errorStatus);
		}
